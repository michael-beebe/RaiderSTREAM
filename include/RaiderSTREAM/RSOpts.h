...
****************************************************/
  ...
  int getNumPEs() const { return numPEs; }
	...
	#if _ENABLE_CUDA_ || _ENABLE_MPI_CUDA_ || _ENABLE_OMP_TARGET_
		bool getThreadBlocks() const { return threadBlocks; }
		bool getThreadsPerBlocks() const { return threadsPerBlock; }
	#endif
...
****************************************************/
  ...
  void setKernelType(RSBaseImpl::RSKernelType type) { kernelType = type; }
	...
	#if _ENABLE_CUDA_ || _ENABLE_MPI_CUDA_ || _ENABLE_OMP_TARGET_
		bool setThreadBlocks(int blocks ) { threadBlocks = blocks; return blocks; }
		bool setThreadsPerBlocks(int threads) { threadsPerBlock = threads; return threads; }
	#endif
