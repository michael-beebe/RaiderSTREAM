 ...
 * @param streamArraySize Size of the stream array.
 **************************************************/
__global__ void seqScale(STREAM_TYPE *__restrict__ d_a,
                         STREAM_TYPE *__restrict__ d_b,
                         STREAM_TYPE *__restrict__ d_c, STREAM_TYPE scalar,
                         ssize_t streamArraySize) {
  ssize_t j = blockIdx.x * blockDim.x + threadIdx.x;
  if (j < streamArraySize)
    d_b[j] = scalar * d_c[j];
 ...
 * @param streamArraySize Size of the stream array.
 **************************************************/
__global__ void seqAdd(STREAM_TYPE *__restrict__ d_a,
                       STREAM_TYPE *__restrict__ d_b,
                       STREAM_TYPE *__restrict__ d_c, ssize_t streamArraySize) {
  ssize_t j = blockIdx.x * blockDim.x + threadIdx.x;
  if (j < streamArraySize)
    d_c[j] = d_a[j] + d_b[j];
}
 ...
 * @param streamArraySize Size of the stream array.
 **************************************************/
__global__ void seqTriad(STREAM_TYPE *__restrict__ d_a,
                         STREAM_TYPE *__restrict__ d_b,
                         STREAM_TYPE *__restrict__ d_c, STREAM_TYPE scalar,
                         ssize_t streamArraySize) {
  ssize_t j = blockIdx.x * blockDim.x + threadIdx.x;
 ...
 **************************************************/
__global__ void
gatherCopy(STREAM_TYPE *__restrict__ d_a, STREAM_TYPE *__restrict__ d_b,
           STREAM_TYPE *__restrict__ d_c, ssize_t *__restrict__ d_IDX1,
           ssize_t *__restrict__ d_IDX2, ssize_t streamArraySize) {
  ssize_t j = blockIdx.x * blockDim.x + threadIdx.x;
  if (j < streamArraySize)
    d_c[j] = d_a[d_IDX1[j]];
}
 ...
 * @param streamArraySize Size of the stream array.
 **************************************************/
__global__ void gatherScale(STREAM_TYPE *__restrict__ d_a,
                            STREAM_TYPE *__restrict__ d_b,
                            STREAM_TYPE *__restrict__ d_c, STREAM_TYPE scalar,
 ...
 **************************************************/
__global__ void
gatherAdd(STREAM_TYPE *__restrict__ d_a, STREAM_TYPE *__restrict__ d_b,
          STREAM_TYPE *__restrict__ d_c, ssize_t *__restrict__ d_IDX1,
          ssize_t *__restrict__ d_IDX2, ssize_t streamArraySize) {
  ssize_t j = blockIdx.x * blockDim.x + threadIdx.x;
  if (j < streamArraySize)
 ...
 * @param streamArraySize Size of the stream array.
 **************************************************/
__global__ void gatherTriad(STREAM_TYPE *__restrict__ d_a,
                            STREAM_TYPE *__restrict__ d_b,
                            STREAM_TYPE *__restrict__ d_c, STREAM_TYPE scalar,
                            ssize_t *__restrict__ d_IDX1,
                            ssize_t *__restrict__ d_IDX2,
 ...
 **************************************************/
__global__ void
scatterCopy(STREAM_TYPE *__restrict__ d_a, STREAM_TYPE *__restrict__ d_b,
            STREAM_TYPE *__restrict__ d_c, ssize_t *__restrict__ d_IDX1,
            ssize_t *__restrict__ d_IDX2, ssize_t streamArraySize) {
  ssize_t j = blockIdx.x * blockDim.x + threadIdx.x;
  if (j < streamArraySize)
 ...
 * @param streamArraySize Size of the stream array.
 **************************************************/
__global__ void scatterScale(STREAM_TYPE *__restrict__ d_a,
                             STREAM_TYPE *__restrict__ d_b,
                             STREAM_TYPE *__restrict__ d_c, STREAM_TYPE scalar,
                             ssize_t *__restrict__ d_IDX1,
                             ssize_t *__restrict__ d_IDX2,
  ...
  ssize_t j = blockIdx.x * blockDim.x + threadIdx.x;
  if (j < streamArraySize)
    d_b[d_IDX2[j]] = scalar * d_c[j];
}

/**************************************************
 * @brief Adds data from two device arrays and stores in a third using scatter
 *operation.
 *
 * @param d_a First source device array.
 * @param d_b Second source device array.
 ...
 **************************************************/
__global__ void
scatterAdd(STREAM_TYPE *__restrict__ d_a, STREAM_TYPE *__restrict__ d_b,
           STREAM_TYPE *__restrict__ d_c, ssize_t *__restrict__ d_IDX1,
           ssize_t *__restrict__ d_IDX2, ssize_t streamArraySize) {
  ssize_t j = blockIdx.x * blockDim.x + threadIdx.x;
  if (j < streamArraySize)
 ...
 * @param streamArraySize Size of the stream array.
 **************************************************/
__global__ void scatterTriad(STREAM_TYPE *__restrict__ d_a,
                             STREAM_TYPE *__restrict__ d_b,
                             STREAM_TYPE *__restrict__ d_c, STREAM_TYPE scalar,
                             ssize_t *__restrict__ d_IDX1,
                             ssize_t *__restrict__ d_IDX2,
  ...
  ssize_t j = blockIdx.x * blockDim.x + threadIdx.x;
  if (j < streamArraySize)
    d_a[d_IDX2[j]] = d_b[j] + scalar * d_c[j];
}
 ...
 * @param streamArraySize Size of the stream array.
 **************************************************/
__global__ void sgCopy(STREAM_TYPE *__restrict__ d_a,
                       STREAM_TYPE *__restrict__ d_b,
                       STREAM_TYPE *__restrict__ d_c,
                       ssize_t *__restrict__ d_IDX1,
                       ssize_t *__restrict__ d_IDX2,
                       ssize_t *__restrict__ d_IDX3, ssize_t streamArraySize) {
  ssize_t j = blockIdx.x * blockDim.x + threadIdx.x;
  if (j < streamArraySize)
 ...
 * @param streamArraySize Size of the stream array.
 **************************************************/
__global__ void sgScale(STREAM_TYPE *__restrict__ d_a,
                        STREAM_TYPE *__restrict__ d_b,
                        STREAM_TYPE *__restrict__ d_c, STREAM_TYPE scalar,
                        ssize_t *__restrict__ d_IDX1,
                        ssize_t *__restrict__ d_IDX2,
 ...
 * @param streamArraySize Size of the stream array.
 **************************************************/
__global__ void sgAdd(STREAM_TYPE *__restrict__ d_a,
                      STREAM_TYPE *__restrict__ d_b,
                      STREAM_TYPE *__restrict__ d_c,
                      ssize_t *__restrict__ d_IDX1,
                      ssize_t *__restrict__ d_IDX2,
 ...
 * @param streamArraySize Size of the stream array.
 **************************************************/
__global__ void sgTriad(STREAM_TYPE *__restrict__ d_a,
                        STREAM_TYPE *__restrict__ d_b,
                        STREAM_TYPE *__restrict__ d_c, STREAM_TYPE scalar,
                        ssize_t *__restrict__ d_IDX1,
                        ssize_t *__restrict__ d_IDX2,
 ...
 * @param streamArraySize Size of the stream array.
 **************************************************/
__global__ void centralCopy(STREAM_TYPE *__restrict__ d_a,
                            STREAM_TYPE *__restrict__ d_b,
                            STREAM_TYPE *__restrict__ d_c,
                            ssize_t streamArraySize) {
  d_c[0] = d_a[0];
 ...
 * @param streamArraySize Size of the stream array.
 **************************************************/
__global__ void centralScale(STREAM_TYPE *__restrict__ d_a,
                             STREAM_TYPE *__restrict__ d_b,
                             STREAM_TYPE *__restrict__ d_c, STREAM_TYPE scalar,
                             ssize_t streamArraySize) {
  d_b[0] = scalar * d_c[0];
 ...
 * @param streamArraySize Size of the stream array.
 **************************************************/
__global__ void centralAdd(STREAM_TYPE *__restrict__ d_a,
                           STREAM_TYPE *__restrict__ d_b,
                           STREAM_TYPE *__restrict__ d_c,
                           ssize_t streamArraySize) {
  d_c[0] = d_a[0] + d_b[0];
 ...
 * @param streamArraySize Size of the stream array.
 **************************************************/
__global__ void centralTriad(STREAM_TYPE *__restrict__ d_a,
                             STREAM_TYPE *__restrict__ d_b,
                             STREAM_TYPE *__restrict__ d_c, STREAM_TYPE scalar,
                             ssize_t streamArraySize) {
