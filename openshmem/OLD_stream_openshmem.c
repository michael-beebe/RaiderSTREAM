...
STREAM_TYPE * restrict gather_a,  * restrict gather_b,  * restrict gather_c;
STREAM_TYPE * restrict scatter_a, * restrict scatter_b, * restrict scatter_c;
...
int main()
{
    ...
    // STREAM_TYPE       AvgError[NUM_ARRAYS];

    /* --- SETUP --- call shmem_init() before anything else! --- */
...
--------------------------------------------------------------------------------------*/
    ...
    gather_a = (STREAM_TYPE*) shmem_align(array_alignment, array_bytes);
    if (gather_a == NULL) {
        printf("Rank %d: Allocation of array gather_a failed!\n",myrank);
        shmem_global_exit(1);
        exit(1);
    }
    gather_b = (STREAM_TYPE*) shmem_align(array_alignment, array_bytes);
    if (gather_b == NULL) {
        printf("Rank %d: Allocation of array gather_b failed!\n",myrank);
        shmem_global_exit(1);
        exit(1);
    }
    gather_c = (STREAM_TYPE*) shmem_align(array_alignment, array_bytes);
    if (gather_c == NULL) {
        printf("Rank %d: Allocation of array gather_c failed!\n",myrank);
        shmem_global_exit(1);
        exit(1);
    }

    scatter_a = (STREAM_TYPE*) shmem_align(array_alignment, array_bytes);
    if (scatter_a == NULL) {
        printf("Rank %d: Allocation of array scatter_a failed!\n",myrank);
        shmem_global_exit(1);
        exit(1);
    }
    scatter_b = (STREAM_TYPE*) shmem_align(array_alignment, array_bytes);
    if (scatter_b == NULL) {
        printf("Rank %d: Allocation of array scatter_b failed!\n",myrank);
        shmem_global_exit(1);
        exit(1);
    }
    scatter_c = (STREAM_TYPE*) shmem_align(array_alignment, array_bytes);
    if (scatter_c == NULL) {
        printf("Rank %d: Allocation of array scatter_c failed!\n",myrank);
        shmem_global_exit(1);
        exit(1);
    }
...
//                         &AvgError[3], &AvgError[4], &AvgError[5],
//                         &AvgError[6], &AvgError[7], &AvgError[8]
//                     ); // FIXME: change the params in computeSTREAMerrors declarations
//
...
// 	}
// 	else if(BytesPerWord == 8){
// 		shmem_fcollect64(AvgErrByRank, AvgError, 3, 0, 0, numranks, psync);
// 	}
// 	else{
// 		printf("ERROR: sizeof(STREAM_TYPE) = %d\n", BytesPerWord);
// 		printf("ERROR: Please set STREAM_TYPE such that sizeof(STREAM_TYPE) = {4,8}\n");
